...
Please see the https://github.com/kigster/puma-daemon/tree/master/examples[`examples`] directory in the source of the gem. It contains `single.sh` and `cluster.sh` scripts that boot Puma via `pumad` binary.
...
❯ pumad -w 3 -C config/puma.rb spec/rackup/bind.ru
